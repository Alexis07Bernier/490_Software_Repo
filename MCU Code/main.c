...
#define DATA_TIMER_INTERVAL APP_TIMER_TICKS(2.5)  /**< Sensors timer interval, in ms. */
...
int main(void) {
  ...
  uint8_t m_mag_value[7];
  uint16_t m_acc_value[3], m_gyro_value[4];
  ...
  while (adis16266_init() == false) {
    // wait for successful initialization
  }
  
  nrf_gpio_cfg_sense_input(2, NRF_GPIO_PIN_PULLUP, NRF_GPIO_PIN_SENSE_LOW); // pin for SW1
  nrf_gpio_cfg_output(0); // pin for indicator LED
...
// Enter main loop.
  for (;;) {
    ...
    if (m_data_flag == true && m_conn_handle != BLE_CONN_HANDLE_INVALID) {
      ...
      if (adxl314_read_accel(m_acc_value) == true) {
      
        NRF_LOG_INFO("ax:%d ay:%d az:%d", m_acc_value[0], m_acc_value[1], m_acc_value[2]);
      }

       if (mmc5983ma_read_mag(m_mag_value) == true)
      {
        NRF_LOG_INFO("Read magnetometer data");
      }
      
      if (itg3701_read_gyro(m_gyro_value) == true)
      {
        NRF_LOG_INFO("gx:%d gy:%d gz:%d", m_gyro_value[0], m_gyro_value[1], m_gyro_value[2]);
      }

      if (adis16266_read_gyro(m_gyro_value) == true) {

        NRF_LOG_INFO("ghz:%d", m_gyro_value[3]);
      }
        ...
        for (uint8_t i = 0; i < (sizeof(m_gyro_value) / 2); i++)
        {
          m_data[index++] = m_gyro_value[i] >> 8;
          m_data[index++] = m_gyro_value[i] & 0xFF;
        }
...
static void ble_evt_handler(ble_evt_t const *p_ble_evt, void *p_context) {
  ...
  case BLE_GAP_EVT_PHY_UPDATE: {
    ...
    err_code = sd_ble_gap_phy_request(p_ble_evt->evt.gap_evt.conn_handle, &phys);
    APP_ERROR_CHECK(err_code);
  } break;
